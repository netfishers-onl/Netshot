...
public abstract class Rule {
	...
	static {
		...
		try {
	    for (Class<?> clazz : NetshotDatabase.listClassesInPackage("org.netshot.compliance.rules")) {
	    	if (Rule.class.isAssignableFrom(clazz)) {
	    		@SuppressWarnings("unchecked")
	        Class<? extends Rule> ruleClass = (Class<? extends Rule>) clazz;
	    		RULE_CLASSES.add(ruleClass);
	    	}
	    }
    }
    catch (Exception e) {
    	logger.error("Error while scanning the rule classes.", e);
    }
	}
