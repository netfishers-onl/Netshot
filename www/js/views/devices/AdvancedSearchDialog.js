define([
	...
	'views/devices/SearchToolbox'
		...
		DeviceConfigFieldCollection, advancedSearchTemplate, SearchToolbox) {
	...
	var AdvancedSearchDialog = Dialog.extend({
		...
		buttons: {
			"Search": function(event) {
				...
				that.devicesView.devices.filter.vendorModel = that.$('#devicetype').val();
			...
			var searchToolbox = new SearchToolbox({
				onRendered: function() {
					if (that.devicesView.devices.filter.type == "advanced" || that.devicesView.devices.filter.type == "simple") {
						that.$('#devicetype').val(that.devicesView.devices.filter.vendorModel);
						that.$('#expression').val(that.devicesView.devices.filter.query);
					}
				}
			});
