 ...
 */
var Info = {
	name: "FortinetFortiAnalyzer", /* Unique identifier of the driver within Netshot. */
	description: "Fortinet FortiAnalyzer", /* Description to be used in the UI. */
	author: "Najihel",
 ...
 */
var Config = {
	"osVersion": { /* This stores the detected FortiAnalyzer OS version on each snapshot. */
		type: "Text",
		title: "FortiAnalyzer OS version",
		comparable: true,
		searchable: true,
		checkable: true,
		dump: {
			pre: "## FortiAnalyzer OS version:",
			preLine: "##  "
		}
	},
...
function snapshot(cli, device, config) {
	...
	var version = status.match(/Version[\s]*: v([0-9]+.*)/);
    var platform = status.match(/Platform Full Name[\s]*: (.*)/);
	var family = (platform ? platform[1] : "FortiAnalyzer device");
	...
	// Store the HA status
	var getHa = cli.command("diagnose ha status");
