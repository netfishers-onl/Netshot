	...
	var ComplianceView = Backbone.View
			.extend({
				...
				render: function() {
					...
					if (!user.isAdmin()) {
						$('#nstoolbar-compliance-addpolicy').remove();
					}
				...
				refreshPolicies: function() {
					var that = this;
					this.policies.fetch().done(function() {
				...
				renderRule: function() {
					var that = this;
					if (this.jsRuleView != null) {
						this.jsRuleView.destroy();
						this.jsRuleView = null;
					}
					if (this.rule != null) {
						this.jsRuleView = new JsRuleView({
							model: this.rule,
							onEdited: function() {
								that.refreshRules();
							}
