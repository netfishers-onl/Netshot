define([
	...
	var EditGroupDialog = Dialog.extend({
	  ...
	  buttons: {
	    "Save": function(event) {
		    ...
		    if (that.model.get('type').match(/\.StaticDeviceGroup$/)) {
			    ...
			    group.deviceClassName = that.$('#devicetype').val();
	  ...
	  renderDynamicGroup: function() {
		  ...
		  var searchToolbox = new SearchToolbox({
			  onRendered: function() {
				  that.$('#devicetype').val(that.model.get('deviceClass'));
		  ...
		  this.$('#preview').button({
			  ...
			  that.searchedDevices.filter = {
			  	...
			  	vendorModel: that.$('#devicetype').val()
			  };
			  that.searchedDevices.fetch().done(function() {
				  that.htmlBuffer = "";
